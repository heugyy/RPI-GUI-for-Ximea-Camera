...
//#include <xiAPIplus/xiapiplus.h>
	...
	// Return device enhanced serial number (for cameras having both serials legacy and enhanced) (XI_PRM_DEVICE_SNE)
	void xiAPIplus_Camera::GetSerialNumberEnhanced(char* buffer, int buffer_length)
	{
		
		CheckCamHandle("GetSerialNumberEnhanced");
		
		XI_RETURN res=xiGetParamString(camera_handle, XI_PRM_DEVICE_SNE, buffer, buffer_length);
		CheckResult(res,"GetSerialNumberEnhanced");
	}
